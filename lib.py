...
def de_en_coder(key,token,state):
	result = list()
	...
	for i,t in enumerate(token_lower) :
		...
		r_index = (state=='encode') and (t_index+k_index) or (t_index-k_index) 
		r_index %= 26
		if token[i].isupper():
...
if __name__ == "__main__" :
	word = "Hello World!"
	key  = "claire"
	word_decoded = de_en_coder(key,word,'encode')
	word_encoded = de_en_coder(key,word_decoded,'decode')
	print word_decoded
	print word_encoded
